...
import numpy as np # For numpy arrays in forecast
...
class BOPDMDOperator(DMDOperator):
    ...
    def _varpro_opts_warn(self):
        ...
        rec_ranges["use_levmarq"] = [-torch.inf, torch.inf]
        rec_ranges["maxiter"] = [0, 1e12]
        rec_ranges["tol"] = [0.0, 1e16]
        rec_ranges["eps_stall"] = [-torch.inf, 1.0]
        rec_ranges["use_fulljac"] = [-torch.inf, torch.inf]
        rec_ranges["verbose"] = [-torch.inf, torch.inf]
    ...
    def _diff_func(eigenvalues, omega, ind, absolute_diff):
        ...
        """
        if absolute_diff:
            diff = torch.abs(torch.abs(eigenvalues.imag) - torch.abs(omega.imag))
            diff[ind] = torch.nan
            return diff

        sign = torch.sign(omega.imag)

        # Catch the edge case of the eigenvalues being exactly 0.
        if sign == 0.0:
            diff = torch.abs(torch.abs(eigenvalues.imag) - omega.imag)
            diff[ind] = torch.nan
            return diff

        same_sign_index = torch.sign(eigenvalues.imag) == sign
        opp_sign_eigs = torch.copy(eigenvalues.imag)
        opp_sign_eigs[same_sign_index] = torch.nan
        return torch.abs(torch.abs(opp_sign_eigs) - torch.abs(omega.imag))
    ...
    def _force_conj_pairs(self, eigenvalues):
        ...
        new_eigs = torch.empty(num_eigs, dtype=self.dtype)

        unassigned_inds = set(torch.arange(num_eigs))
        pair_indices = torch.zeros((num_eigs // 2, 2))
        pair_counter = 0
        num_expected_pairs = num_eigs

        diff_array = torch.zeros((num_eigs, num_eigs))

        # If given an odd number of eigenvalues, find the eigenvalue with
        # the smallest imaginary part and take it to be a DC mode
        # eigenvalue.
        if num_eigs % 2 == 1:
            ind_0 = torch.argmin(torch.abs(eigenvalues.imag))
            new_eigs[ind_0] = eigenvalues[ind_0].real + 0j
            unassigned_inds.remove(ind_0)
            eigenvalues[ind_0] = torch.nan + 1j * torch.nan
        ...
        absolute_diff = torch.count_nonzero(
            eigenvalues.imag > 0
        ) != torch.count_nonzero(eigenvalues.imag < 0)
        ...
        while unassigned_inds:
            ...
            ind_1, ind_2 = torch.nonzero(diff_array == torch.nanmin(diff_array))
            ind_1 = ind_1[0]
            ind_2 = ind_2[0]
            ...
            diff_array[ind_1] = torch.nan
            diff_array[:, ind_1] = torch.nan
            diff_array[:, ind_2] = torch.nan
            diff_array[ind_2] = torch.nan
            ...
            b = 0.5 * (torch.abs(eig_1.imag) + torch.abs(eig_2.imag))
            ...
            sign_1 = torch.sign(eig_1.imag)
            sign_2 = torch.sign(eig_2.imag)
            if sign_1 == sign_2:
        ...
        if not len(torch.unique(pair_indices)) == num_eigs_adj:
    ...
    def _push_eigenvalues(self, eigenvalues):
        ...
        if "stable" in self._eig_constraints:
            ...
            too_big = torch.abs(eigenvalues.real) > self._real_eig_limit
    ...
    def _exp_function(self, alpha, t):
        ...
        :rtype: numpy.ndarray
        """
        return torch.exp(torch.outer(t, alpha))
    ...
    def _exp_function_deriv(self, alpha, t, i):
        ...
        A = torch.multiply(t, torch.exp(alpha[i] * t))
        # return csr_matrix(
        #     (A, (torch.arange(m), torch.full((m,), fill_value=i))), shape=(m, n)
        # )
        out = torch.zeros((m, n), dtype=self.dtype)
        out[:, i] = A
        return out
    ...
    def _compute_irank_svd(X, tolrank):
        ...
        irank = torch.sum(s > tolrank * s[0])
        U = U[:, :irank]
        S = torch.diag(s[:irank])
        Vh = Vh[:irank]
        return U, S, Vh
    ...
    def _argsort_eigenvalues(self, eigs):
        ...
        if self._eig_sort == "auto":
            real_var = torch.var(eigs.real)
            imag_var = torch.var(eigs.imag)
            abs_var = torch.var(torch.abs(eigs))
            all_var = [real_var, imag_var, abs_var]
            self._eig_sort = ("real", "imag", "abs")[torch.argmax(all_var)]

        # Sort the results according to eig_sort.
        if self._eig_sort == "real":
            return torch.argsort(eigs)
        if self._eig_sort == "imag":
            eigs_real_imag_swapped = eigs.imag + (1j * eigs.real)
            return torch.argsort(eigs_real_imag_swapped)
        if self._eig_sort == "abs":
            return torch.argsort(torch.abs(eigs))
    ...
    def _bag(self, H, trial_size):
        ...
        all_inds = torch.arange(H.shape[0])
        subset_inds = torch.sort(
            torch.random.choice(all_inds, size=batch_size, replace=False)
        )
    ...
    def _variable_projection(
        ...
        print(f"Projecting H with shape {H.shape}, t with shape {t.shape}, and init_alpha with shape {init_alpha.shape}...\n")
        """
        ...
        M, IS = H.shape
        IA = len(init_alpha)
        tolrank = M * torch.finfo(float).eps
        ...
        def compute_error(B, alpha):
            ...
            """
            residual = H - (Phi(alpha, t) @ B)
            objective = 0.5 * torch.linalg.norm(residual, "fro") ** 2
            error = torch.linalg.norm(residual, "fro") / torch.linalg.norm(H, "fro")
        ...
        def compute_B(alpha, amp_lim=None):
            ...
            try:
                A = Phi(alpha, t)
                B = torch.linalg.lstsq(A, H.type(A.dtype), rcond=None)[0]
                print(f"Computed B with shape {B.shape} from A with shape {A.shape} and H with shape {H.shape}")
            except LinAlgError:
                msg = (
            ...
            if amp_lim is not None:
                b = torch.sqrt(torch.sum(torch.abs(B) ** 2, axis=1))
                B[b > amp_lim] = torch.finfo(float).eps
        ...
        # Initialize storage.
        all_error = torch.zeros(maxiter)
        djac_matrix = torch.zeros((M * IS, IA), dtype=self.dtype)
        rjac = torch.zeros((2 * IA, IA), dtype=self.dtype)
        scales = torch.zeros(IA)
        ...
        for itr in range(maxiter):
            ...
            for i in range(IA):
                ...
                ut_dphi = (torch.conj(U).swapaxes(-2, -1) @ dphi_temp)
                uut_dphi = (U @ ut_dphi)
                djac_a = (dphi_temp - uut_dphi) @ B
                djac_matrix[:, i] = djac_a.T.ravel()

                # Compute the full expression for the Jacobian.
                if use_fulljac:
                    transform = torch.linalg.multi_dot([U, torch.linalg.inv(S).type(U.dtype), Vh])
                    dphit_res = (dphi_temp.conj().T @ residual)
                    djac_b = transform @ dphit_res
                    print(f"\nComputed djac_b with shape {djac_b.shape}\n")
                    djac_matrix[:, i] += djac_b.T.type(self.dtype).contiguous().ravel()

                # Scale for the Levenberg algorithm.
                scales[i] = 1
                # Scale for the Levenberg-Marquardt algorithm.
                if use_levmarq:
                    scales[i] = min(torch.linalg.norm(djac_matrix[:, i]), 1)
                    scales[i] = max(scales[i], 1e-6)

            # Loop to determine lambda (the step-size parameter).
            rhs_temp = residual.T.ravel().copy_(residual.T.ravel())[:, None]
            q_out, djac_out, j_pvt = qr(
                djac_matrix.cpu().numpy(), mode="economic", pivoting=True
            )
            q_out, djac_out, j_pvt = torch.from_numpy(q_out).type(self.dtype), torch.from_numpy(djac_out).type(self.dtype), torch.from_numpy(j_pvt)
            if not self._varpro_flag:
                ...
                ij_pvt = torch.arange(IA)
                ij_pvt = ij_pvt[j_pvt]
            elif self._varpro_flag:
                # Use the true variable projection.
                ij_pvt = torch.zeros(IA, dtype=int)
                ij_pvt[j_pvt] = torch.arange(IA, dtype=int)
            rjac[:IA] = torch.triu(djac_out[:IA])
            rhs_top = q_out.conj().T @ rhs_temp
            scales_pvt = scales[j_pvt[:IA]]
            rhs = torch.concatenate(
                (rhs_top[:IA].flatten(), torch.zeros(IA, dtype=self.dtype))
            )
            ...
            def step(
                ...
                rjac[IA:] = _lambda * torch.diag(scales_pvt)
                delta = torch.linalg.lstsq(rjac.type(self.dtype), rhs, rcond=None)[0]
                delta = delta[ij_pvt]
    ...
    def _single_trial_compute_operator(self, H, t, init_alpha):
        ...
        """
        print(f"Computing single trial operator with H with shape {H.shape}, t with shape {t.shape}, and init_alpha with shape {init_alpha.shape}...\n")
        ...
        w = B.T
        e = alpha
        b = torch.sqrt(torch.sum(torch.abs(w) ** 2, axis=0))
        print(f"Sizes of w, e, b are {w.shape}, {e.shape}, {b.shape}")

        # Normalize the modes and the amplitudes.
        inds_small = torch.abs(b) < (10 * torch.finfo(float).eps * torch.max(b))
        b[inds_small] = 1.0
        w = w @ torch.diag(1 / b.type(self.dtype))
        w[:, inds_small] = 0.0
        b[inds_small] = 0.0

        # Compute the projected propagator Atilde.
        if self._use_proj:
            Atilde = torch.linalg.multi_dot([w, torch.diag(e), torch.linalg.pinv(w)])
            # Unproject the dmd modes.
            w = self._proj_basis.type(self.dtype) @ w
            # Apply mode proximal operator if given.
            if self._mode_prox is not None:
                w = self._mode_prox(w)
        else:
            w_proj = self._proj_basis.conj().T @ w
            Atilde = torch.linalg.multi_dot(
                [w_proj, torch.diag(e), torch.linalg.pinv(w_proj)]
            )

        # Compute the full system matrix A.
        if self._compute_A:
            A = torch.linalg.multi_dot([w, torch.diag(e), torch.linalg.pinv(w)])
        else:
            A = None
    ...
    def compute_operator(self, H, t):
        ...
        :rtype: numpy.ndarray
        """
        print(f"Computing operator with H with shape {H.shape}, self._proj_basis with shape {self._proj_basis.shape}, t with shape {t.shape}...\n")
        ...
        w_sum = torch.zeros(w_0.shape, dtype="complex")
        e_sum = torch.zeros(e_0.shape, dtype="complex")
        b_sum = torch.zeros(b_0.shape, dtype="complex")
        w_sum2 = torch.zeros(w_0.shape, dtype="complex")
        e_sum2 = torch.zeros(e_0.shape, dtype="complex")
        b_sum2 = torch.zeros(b_0.shape, dtype="complex")
        ...
        while num_successful_trials < self._num_trials:
            ...
            if converged or not self._remove_bad_bags:
                ...
                w_sum2 += torch.abs(w_i[:, sorted_inds]) ** 2
                e_sum2 += torch.abs(e_i[sorted_inds]) ** 2
                b_sum2 += torch.abs(b_i[sorted_inds]) ** 2
        ...
        w_std = torch.sqrt(torch.abs(w_sum2 / self._num_trials - torch.abs(w_mu) ** 2))
        e_std = torch.sqrt(torch.abs(e_sum2 / self._num_trials - torch.abs(e_mu) ** 2))
        b_std = torch.sqrt(torch.abs(b_sum2 / self._num_trials - torch.abs(b_mu) ** 2))
        ...
        self._Atilde = torch.linalg.multi_dot(
            [w_proj, torch.diag(self._eigenvalues), torch.linalg.pinv(w_proj)]
        )

        # Compute A if requested.
        if self._compute_A:
            self._A = torch.linalg.multi_dot(
                [
                    self._modes,
                    torch.diag(self._eigenvalues),
                    torch.linalg.pinv(self._modes),
                ]
            )
...
class BOPDMD(DMDBase):
    ...
    def __init__(
        ...
        self._b = None # Initialize amplitudes
        self.operator = None # Initialize operator
        print("BOPDMD.__init__: Initialized BOPDMD instance")
        flush_print()
    ...
    def dynamics(self):
        ...
        :rtype: numpy.ndarray
        """
        t_omega = torch.exp(torch.outer(self.eigs, self.time))
        return torch.diag(self.amplitudes).dot(t_omega)
    ...
    def _check_eig_constraints(self, eig_constraints):
        ...
        if isfunction(eig_constraints):
            ...
            try:
                # Test the eig_constraints function on a (r,) torch.Tensor.
                x_dummy = torch.arange(r)
            ...
            if not isinstance(y_dummy, torch.Tensor):
                msg = (
    ...
    def _initialize_alpha(self, s=None, V=None):
        ...
        if s is not None and V is not None:
            ...
            ux = torch.diag(s).dot(V)
            print(f"Using provided singular values and right singular vectors with shapes {s.shape} and {V.shape}.\n")
        else:
            # Project the snapshot data onto the projection basis.
            print(f"Using projection basis with shape {self._proj_basis.shape}.\n")
            ux = self._proj_basis.conj().T @ self.snapshots
        ux1 = ux[:, :-1]
        ux2 = ux[:, 1:]
        ...
        print()
        print(f"U.shape: {U.shape}, V.shape: {V.shape}, S.shape: {S.shape}, Z.shape: {Z.shape}")
        Atilde = torch.linalg.multi_dot([U.conj().T, Z, V, torch.linalg.inv(S)])

        return torch.linalg.eig(Atilde)[0]
    ...
    def fit(self, X, t):
        ...
        self._time = torch.Tensor(t).squeeze()
        ...
        if self._proj_basis is None and self._use_proj:
            ...
            not isinstance(self._proj_basis, torch.Tensor)
            or self._proj_basis.ndim != 2
            or self._proj_basis.shape[1] != self._svd_rank
        ):
            msg = "proj_basis must be a 2D torch.Tensor with {} columns."
        ...
        if self._init_alpha is None:
            ...
            not isinstance(self._init_alpha, torch.Tensor)
            or self._init_alpha.ndim > 1
            or len(self._init_alpha) != self._svd_rank
        ):
            msg = "init_alpha must be a 1D torch.Tensor with {} entries."
            raise ValueError(msg.format(self._svd_rank))
        ...
        if self._use_proj:
            snp = self._proj_basis.conj().T @ self.snapshots
        else:
            snp = self.snapshots
        
        print(f"Using snp of shape {snp.shape}\n")

        # Fit the data.
        print("BOPDMD.fit: Calling self.operator.compute_operator...")
        flush_print()
        self._b = self.operator.compute_operator(snp.T, self._time)
        print(f"BOPDMD.fit: compute_operator returned _b of shape {self._b.shape}\n")
        print("BOPDMD.fit: Final computed eigenvalues (self.eigs):", self.eigs[:5] if len(self.eigs) > 5 else self.eigs)
        print("BOPDMD.fit: Final computed amplitudes (self.amplitudes):", self.amplitudes[:5] if len(self.amplitudes) > 5 else self.amplitudes)
        print(f"BOPDMD.fit: Final computed modes shape (self.modes): {self.modes.shape}")
        flush_print()
    ...
    def fit_econ(self, s, V, t):
        ...
        """
        self._reset()
        self._time = torch.asarray(t).squeeze()
        s = torch.asarray(s).squeeze()
        ...
        if (
            ...
            msg = (
                "proj_basis must be a 2D torch.Tensor with "
    ...
    def forecast(self, t):
        ...
        """
        print(f"BOPDMD.forecast: Received t of shape {np.asarray(t).squeeze().shape}")
        flush_print()
        ...
        # Access operator directly if available
        op = self.operator
        if op is not None and op.eigenvalues_std is not None:
            print("BOPDMD.forecast: Calculating forecast with variance...")
            flush_print()
            # Compute num_trials many forecasts.
            # Ensure self.snapshots is available; might need a check or alternative
            if self.snapshots is None:
                 raise ValueError("Snapshots not available for forecast dimension calculation.")
            num_features = self.snapshots.shape[0]
            num_trials = self._num_trials # Use the stored num_trials

            # Initialize storage for all forecasts
            all_x = np.empty(
                (num_trials, num_features, len(t)),
                dtype="complex",
            )

            # Pre-calculate means and stds to avoid repeated property access
            eigs_mean = self.eigs
            b_mean = self.amplitudes
            modes_mean = self.modes
            eigs_std = op.eigenvalues_std
            b_std = op.amplitudes_std

            for k in range(num_trials):
                # Draw eigenvalues and amplitudes from random distribution.
                # Use numpy's random number generator for numpy arrays
                eigs_k = eigs_mean + np.random.randn(*eigs_mean.shape) * eigs_std
                b_k = b_mean + np.random.randn(*b_mean.shape) * b_std

                # Compute forecast using average modes and sampled eigs_k, b_k.
                # Ensure complex dtype for intermediate calculations if needed
                time_evolution_k = np.exp(np.outer(eigs_k, t))
                dynamics_k = np.diag(b_k) @ time_evolution_k
                all_x[k] = modes_mean @ dynamics_k

            # Return the average forecast and the variance.
            forecast_mean = np.mean(all_x, axis=0)
            forecast_var = np.var(all_x, axis=0)
            print(f"BOPDMD.forecast: Returning mean shape {forecast_mean.shape}, var shape {forecast_var.shape}")
            flush_print()
            return forecast_mean, forecast_var

        # If no variance information, simply compute a standard forecast.
        print("BOPDMD.forecast: Calculating deterministic forecast...")
        flush_print()
        time_evolution = np.exp(np.outer(self.eigs, t))
        dynamics = np.diag(self.amplitudes) @ time_evolution
        x = self.modes @ dynamics
        print(f"BOPDMD.forecast: Returning deterministic forecast shape {x.shape}")
        flush_print()
        return x
    ...
    def plot_mode_uq(
        ...
        *,
        ...
        :param x: Points along the 1st spatial dimension where data has
            been collected.
        :type x: torch.Tensor or iterable
        ...
        :type y: torch.Tensor or iterable
        ...
        if plot_modes is None or isinstance(plot_modes, int):
            mode_indices = torch.arange(r)
            if not plot_conjugate_pairs:
                if r % 2 == 0:
                    mode_indices = mode_indices[::2]
                else:
                    mode_indices = torch.concatenate([(0,), mode_indices[1::2]])
    ...
    def plot_eig_uq(
        ...
        :type eigs_true: torch.Tensor or iterable
        ...
        if eigs_true is not None:
            eigs_true = torch.array(eigs_true)
        ...
        plt.legend()
        # Removed plt.show() to allow saving from test script
        # plt.show() # This would block execution if called here
